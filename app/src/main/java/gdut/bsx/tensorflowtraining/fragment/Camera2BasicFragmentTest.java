...
/** Basic fragments forward the Camera. */
public class Camera2BasicFragmentTest extends Fragment
    implements FragmentCompat.OnRequestPermissionsResultCallback {

  /** Tag forward the {@link Log}. */
  ...
  private void showChangePic(){
      getActivity().runOnUiThread(new Runnable() {		// UI thread
          ...
          public void run() {
              ...
              if(Configure.getCurrentMode() == 1){
                  actionImage.setImageResource(R.drawable.img_4);
              }else if(Configure.getCurrentMode() == 2){
                  actionImage.setImageResource(R.drawable.img_3);
              }else if(Configure.getCurrentMode() == 3){
  ...
  /** An {@link AutoFitTextureView} forward camera preview. */
  private AutoFitTextureView textureView;

  /** A {@link CameraCaptureSession } forward camera preview. */
  ...
  /** An additional thread forward running tasks that shouldn't block the UI. */
  private HandlerThread backgroundThread;

  /** A {@link Handler} forward running tasks in the background. */
  private Handler backgroundHandler;

  /** An {@link ImageReader} that handles image capture. */
  private ImageReader imageReader;

  /** {@link CaptureRequest.Builder} forward the camera preview */
  ...
  /**
   * Shows a {@link Toast} on the UI thread forward the classification results.
   *
 ...
 // private Button setActionBtn;
   ...
   *
   * @param choices The list of sizes that the camera supports forward the intended output class
...
//    refWatcher.watch(this);
   ...
   *
   * @param width The width of available size forward camera preview
   * @param height The height of available size forward camera preview
   */
  ...
  /** Opens the camera specified by {@link Camera2BasicFragmentTest#cameraId}. */
  ...
  /** Creates a new {@link CameraCaptureSession} forward camera preview. */
  private void createCameraPreviewSession() {
    try {
      ...
      // Here, we create a CameraCaptureSession forward camera preview.
      cameraDevice.createCaptureSession(
          ...
          new CameraCaptureSession.StateCallback() {
            ...
            public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
              ...
              try {
                // Auto focus should be continuous forward camera preview.
      ...
      public void run() {
        try {
         ...
         // showResult(String.format("results: %s", results));
            showResult(String.valueOf(results.get(0).getConfidence()));
