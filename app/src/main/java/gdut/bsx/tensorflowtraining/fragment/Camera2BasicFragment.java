...
//          showResult(String.format("results: %s", results));
    ...
    private void countScore(List<Classifier.Recognition> result) {
      ...
      if(isShowToast && curPic != 8){
          ...
          Log.e("isShowToast", "i: "+i );

          getActivity().runOnUiThread(new Runnable() {
              @Override
              public void run() {
//                  excellentIv.setImageResource(R.drawable.excellent);
                  if(i % 5 == 0){
                      Log.e("isShowToast", "0" );
                      excellentIv.setImageResource(R.drawable.excellent);
                  }else if(i % 5 == 1){
                      Log.e("isShowToast", "1" );
                      excellentIv.setImageResource(R.drawable.cool);
                  }else if(i % 5 == 2){
                      Log.e("isShowToast", "2" );
                      excellentIv.setImageResource(R.drawable.bad);
                  }else if(i % 5 == 3){
                      Log.e("isShowToast", "3" );
                      excellentIv.setImageResource(R.drawable.good);
                  }else if(i % 5 == 4){
                      Log.e("isShowToast", "4" );
                      excellentIv.setImageResource(R.drawable.perfect);
                  }
                  excellentIv.setVisibility(View.VISIBLE);

                  Handler handler = new Handler();
                  handler.postDelayed(new Runnable() {
                      @Override
                      public void run() {
                          excellentIv.setVisibility(View.GONE);
                      }
                  },2000);
              }
          });
